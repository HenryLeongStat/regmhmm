...
test_that("VHS-HMM should give better estimates than HMM", {
  ...
  VNS_ML_only_without_regular <- VNS_HMM(
    delta = delta_start,
    Y_mat = dat$y_mat,
    A = trans_mat_start,
    B = emiss_mat_start[, 1:4],
  ...
  for(i in 1:length(VNS_ML_only_without_regular)){
    get_all_llh <- c(get_all_llh, VNS_ML_only_without_regular[[i]]$log_likelihood)
  }
  
  # initial prob
  # cat("checking intial prob:\n")
  expect_equal(max(get_all_llh)>=ML_only_without_regular$log_likelihood, TRUE)
})
