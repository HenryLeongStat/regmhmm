...
simulate_mHMM_data_with_name <- function(seed_num,
                              p_noise,
                              N,
                              N_persub,
                              parameters_setting) {
  ...
  agg_mat_big <- matrix(NA, 
                        nrow=dim(y_mat)[1]*dim(y_mat)[2],
                        ncol=p+p_noise+1+1)
  
  for(i in 1:dim(y_mat)[1]){
    agg_mat_big[(1+(i-1)*N_persub):(N_persub+(i-1)*N_persub),1:(p+p_noise)] <- X_array[,-1,i]
    agg_mat_big[(1+(i-1)*N_persub):(N_persub+(i-1)*N_persub),p+p_noise+1] <- y_mat[i,]
    agg_mat_big[(1+(i-1)*N_persub):(N_persub+(i-1)*N_persub),p+p_noise+1+1] <- seq(1, N_persub, by=1)
  }
  
  agg_mat_big <- as.data.frame(agg_mat_big)

  names(agg_mat_big)[1:(p+p_noise)] <- paste0("column", 1:(p+p_noise))
  names(agg_mat_big)[p+p_noise+1] <- "response"
  names(agg_mat_big)[p+p_noise+1+1] <- "time"
  
  agg_mat_big$ID <- rep(1:N, each=N_persub)
  return(agg_mat_big)
}
